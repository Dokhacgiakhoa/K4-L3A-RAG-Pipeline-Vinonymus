...
|---|---|---|---|
| Data Collection (Legal & News) | Thu thập các tài liệu văn bản quy chế chính thức: Sổ tay học viên v2.1, Chính sách tuyển sinh, Lịch sử triển khai và 9 bài viết tin tức | `src/task1_collect_legal_docs.py`, `src/task2_crawl_news.py`, PR #1 | Done |
...
| Corpus Quality Testing | Viết bài test chất lượng dữ liệu đầu vào kiểm tra độ dài và tính toàn vẹn | `tests/test_corpus_quality.py` | Done |
...
2. **Quyết định:** Viết test suite `test_corpus_quality.py` độc lập để validate tập tài liệu trước khi đưa vào chunking.  
...
- Test hoặc query tôi đã dùng: `pytest tests/test_corpus_quality.py` và `pytest tests/test_acceptance.py`.
